...
public struct AnimatedWaveformView: View {
    ...
    public var body: some View {
        GeometryReader { geo in
            ...
            AnimatingWaveform(lineLengths: lineLengths)
...
struct AnimatingWaveform: InsettableShape {
    ...
    var animatableData: AnimatablePair<CGFloat, AnimatablePair<CGFloat, AnimatablePair<CGFloat, AnimatablePair<CGFloat, AnimatablePair<CGFloat, CGFloat>>>>> {
        ...
        set {
            lineLengths[0] = CGFloat(newValue.first)
            lineLengths[1] = CGFloat(newValue.second.first)
            lineLengths[2] = CGFloat(newValue.second.second.first)
            lineLengths[3] = CGFloat(newValue.second.second.second.first)
            lineLengths[4] = CGFloat(newValue.second.second.second.second.first)
            lineLengths[5] = CGFloat(newValue.second.second.second.second.second)
        }
    }
    ...
    func path(in rect: CGRect) -> Path {
        ...
        let lines: [[CGPoint]] = [
            [CGPoint(x: w*25, y: rect.midY - h*lineLengths[0]),
             CGPoint(x: w*25, y: rect.midY + h*lineLengths[0])],
            [CGPoint(x: w*35, y: rect.midY - h*lineLengths[1]),
             CGPoint(x: w*35, y: rect.midY + h*lineLengths[1])],
            [CGPoint(x: w*45, y: rect.midY - h*lineLengths[2]),
             CGPoint(x: w*45, y: rect.midY + h*lineLengths[2])],
            [CGPoint(x: w*55, y: rect.midY - h*lineLengths[3]),
             CGPoint(x: w*55, y: rect.midY + h*lineLengths[3])],
            [CGPoint(x: w*65, y: rect.midY - h*lineLengths[4]),
             CGPoint(x: w*65, y: rect.midY + h*lineLengths[4])],
            [CGPoint(x: w*75, y: rect.midY - h*lineLengths[5]),
             CGPoint(x: w*75, y: rect.midY + h*lineLengths[5])]
        ]
